
# Images
This tutorial explains how to include images and image-buttons into your SwiftGUI user interface.
It also explains how to change the window-icon (top left corner of the window).

You'll also learn, how to include images into the layout, without "carrying the image-file everywhere the script goes" (aka accessing an actual image-file).
That is useful when converting the script to an executable file (`.exe`).

# "Pure" image-element
To add an image to the layout, use the `sg.Image`-element 
(`python-logo.png` is placed in the same folder as the script):
```py
import SwiftGUI as sg

### Global options ###


### Layout ###
layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
            "python-logo.png"   # Path to the image
        )
    ]
]

w = sg.Window(layout)

### Additional configurations/actions ###


### Main loop ###
for e,v in w:
    ...

### After window was closed ###
```
![](../assets/images/2025-08-19-23-26-23.png)

As you can see, you may select an image by its name/path.

SwiftGUI supports any format `pillow` (the underlying Python-package for image-handling) supports.
This includes most certainly any format you'll ever use, **except `.svg`** (sadly).

## Other ways to add images
There are a couple of other ways to provide an image to your layout.

Like many thingy in Python, this goes by the motto "if it might work, it works".

### As a "normal" file
Open the image as a file in binary mode (`rb`):
```py
    sg.Image(
        open("python-logo.png", "rb")
    )
```

### As a pillow (PIL) - image
The pillow library is used by SwiftGUI for handling images, so it was installed with SwiftGUI already.

Pillow is great and popular for manipulating images, so SwiftGUI lets you integrate its image format seamlessly:
```py
from PIL import Image

img = Image.open("python-logo.png") # Open the image with pillow (PIL)

layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
            img # Pass the image
        )
    ]
]
```

### Integrating an image without keeping its file
Converting a python-script to `.exe` (executable file of Windows) does not magically include referenced image-file into the exe-file.

That means, all referenced image-files must still be available, including window-icons.
Instead of a single `.exe`-file, the user needs to carry around a whole folder with all images
Annoying.

To solve that issue, you may integrate images into your python-file (`.py`) directly.

Don't do this with larger images without resizing, or the scripts will become pretty big.

Use [this tool](https://github.com/CheesecakeTV/SwiftGUI/blob/main/Tools/Image_To_Base64.py) to encode images base64.
I won't go into detail on how this actually works, I'll just tell you how to use it.

When executing the tool-script, this window will open (looks a bit different now, version 0.10.13):
![](../assets/images/2025-08-20-00-02-50.png)

The layout is pretty straight-forward.
Just click on `Choose file(s)` and select all the images you'd like to convert.

Click `Convert` and the base64-strings are output to the text-field:\
![](../assets/images/2025-08-20-00-07-16.png)

Copy all of the text and paste it into your python-script.
You might need to adjust the variable-names though.

**Note that the images got resized to 30x30 pixels**. To not resize, just leave the two inputs empty:\
![](../assets/images/2025-08-20-00-10-04.png)

To use these byte-strings, you'll need to convert them to something simmilar to an open file.\
Use `sg.file_from_b64` to do so:
```py
python_logo: bytes = b''

layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
            sg.file_from_b64(python_logo)   # Convert the binary-string to a "fake-file" and pass it to the image
        )
    ]
]
```

## Resizing images
`sg.Image` and `sg.ImageButton` take `image_width` and `image_height` as options.

If you define neither, the image doesn't get resized.

Defining either `image_width`, or `image_height` (but not both), will resize the image, while keeping its aspect-ratio intact:
```py
layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
            "python-logo.png",
            image_width=300,
        ),
    ],[
        sg.Image(
            "python-logo.png",
            image_width=200,
        ),
    ],[
        sg.Image(
            "python-logo.png",
            image_width=100,
        ),
    ],[
        sg.Image(
            "python-logo.png",
            image_width=50,
        ),
    ],
]
```
![](../assets/images/2025-08-20-00-30-21.png)

Defining both dimensions might force a different aspect-ratio onto the image, messing up how it looks:
```py
layout:list[list[sg.BaseElement]] = [
    [
        sg.Image(
            "python-logo.png",
            image_width=150,
            image_height=150,
        ),
    ]
]
```
![](../assets/images/2025-08-20-00-33-21.png)

## .update
You can change `image`, `image_width` and `image_height` using `.update(...)` too.

If you change the `image` that way, `image_width` and `image_height` will be automatically reapplied, possibly resizing the image.

For other behavior, you'll need to resize the image "manually" using pillow (PIL) for now.
There might be some more features for this in the future.

# Image-button
`sg.ImageButton` works like a normal button, but displays an image:
```py
layout:list[list[sg.BaseElement]] = [
    [
        sg.ImageButton(
            "python-logo.png",
            key= "ImageClicked",
            image_width= 100,
        ),
    ]
]
```
![](../assets/images/2025-08-20-00-40-48.png)

It has pretty much the same options as normal `sg.Button` combined with `sg.Image`.

## Adding text to the image-button
(This part was written in SwiftGUI version 0.8.2, much later than the rest of this tutorial.)

Image-buttons are quite useful, but displaying no text on a button might be hard for the user to understand. 

Sure, you could edit the image adding text manually, but that's quite an annoyance.

That's why the image-button has the option `text`, which lets you add text next to the image:
```py
sg.Themes.FourColors.CocoaMilk()    # Add a theme so it looks good

layout = [
    [
        sg.ImageButton(
            "Star.png",
            text= "Star!",
        )
    ]
]

w = sg.Window(layout, padx=150, pady=50)    # Add some space around the button using padx and pady
```
![](../assets/images/2025-09-21-18-25-52.png)

A feature to change the color of monochromatic images is planned, but not implemented yet (version 0.10.3).

Change the location of the image relative to the text by defining `compound`:
```py
layout = [
    [
        sg.ImageButton("Star.png",text= "right", compound="right"),
        sg.ImageButton("Star.png", text="left", compound="left"),
        sg.ImageButton("Star.png", text="bottom", compound="bottom"),
        sg.ImageButton("Star.png", text="top", compound="top"),
    ]
]
```
![](../assets/images/2025-09-21-18-30-34.png)

## Changing the button-size
We talked about resizing the image, but what about the actual button around the image?

Usually, `width` represents the number of characters in a row and `height` the number of rows.
For image-buttons, both of these values are given in pixels:
```py
layout = [
    [
        sg.ImageButton("Star.png",text= "right", compound="right", width=100, height=50),
        sg.ImageButton("Star.png", text="left", compound="left", width=100, height=50),
        sg.ImageButton("Star.png", text="bottom", compound="bottom", width=100, height=50),
        sg.ImageButton("Star.png", text="top", compound="top", width=100, height=50),
    ]
]
```
![](../assets/images/2025-09-21-18-36-22.png)

This might feel annoying, but trust me, the alternatives are way worse.

# Window-icon
To set a window-icon, add `icon` as an option to the `sg.Window`:
```py
layout:list[list[sg.BaseElement]] = [
    [
        sg.ImageButton(
            "python-logo.png",
            key= "ImageClicked",
            image_width= 100,
        ),
    ]
]

w = sg.Window(layout, icon="python-logo-small.jpeg")    # Add the .jpeg file as an icon
```
![](../assets/images/2025-08-20-00-52-19.png)

`icon` can be modified using `w.update(icon = ...)` too and accepts any image-format `sg.Image` also accepts.

