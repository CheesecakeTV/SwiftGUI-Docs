...
SwiftGUI supports any format `pillow` (the underlying package for image-handling) supports.
...
Like many thingy in Python, this goes by the motto "if it might fit, it fits".
...
Converting a python-script to `.exe` (executable file of Windows) does not magically integrate referenced image-file into the exe.

That means, all referenced image-files must be available, including window-icons.
Annoying.
...
Of course, you could just integrate the text directly into the image, but that is going to take a lot of work and can't be modified easily.
