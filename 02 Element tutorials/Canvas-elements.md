...
# Canvas-elements
This tutorial is only about the different types of canvas-elements.
If you want to know what you can do with them, check out the tutorial on the canvas itself.
...
This feature can come in handy when two stippled patterns are right next to each other.
A wrong stippleoffset might cause visual errors.
...
## Angle
To define what part is "cut out", define the `start_angle` and `extend_angle` option:
```py
canv = sg.Canvas(
    sgc.Arc(
        (10, 10),
...
However, if the text has long lines, the text might exceed that boundary.
...
If this was something like a value-field, it would probably look asymmetrical.
...
What I find pretty cool is that you may set `bitmap_active` and `bitmap_disabled`, so the bitmap changes when active/disabled.
...
`sgc.Element` is NOT a baseclass of sgc-elements, but a real sgc-element.

It lets you attach a single sg-element to itself:
```py
canv = sg.Canvas(
    sgc.Element(
        (50, 50),
...
Key-wise, these sg-elements ignore the canvas and pass up the events to the next higher key-handler (window, sublayout, subwindow).
